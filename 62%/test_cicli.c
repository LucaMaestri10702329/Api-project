...
void word_checker_3(char *rif, char *word,unsigned int dim, char *vincoli, int non_presente[], int presente_n[][64],int presente_pos_sbagliata[][320], int * ultima_pos_inserita) {
    ...
    for(i=0;i<dim-1;i++) {
        if(output[i]=='+')
            ...
            if(output[i]=='|') {
                ...
                presente_pos_sbagliata[0][*ultima_pos_inserita] = word[i];
                presente_pos_sbagliata[1][*ultima_pos_inserita] = i;
                *ultima_pos_inserita =  *ultima_pos_inserita +1;
            }
            else {
...
unsigned int applica_vincoli_vettori_single_word(char* input, unsigned int dim, char *vincoli, int non_presente[],int presente_n[][64],int presente_pos_sbagliata[][320]) {
    unsigned int i;
    for (i = 0; i < dim - 1; i++) {
...
void ripristina_vincoli_vettori( int presente_n[][64],int dim,int presente_pos_sbagliata[][320],int non_presente[]) {
    unsigned int m,n;
        ...
        for(n=0;n<320;n++){
            presente_pos_sbagliata[0][n] = -1;
            presente_pos_sbagliata[1][n] = -1;
    }
    for(m=0;m<320;m++) {
        non_presente[m] = -1;
    }
...
/*    clock_t t;
    ...
    int ultimo_accesso_pos_sbgliata=0;
                ...
                if (par_let[0] == '+') {
                    ...
                    if (ricerca_parola_2(bt, par_let,dim) == NULL)
                    {
                        ...
                        if (strcmp_home_made(p, par_let) == 0) {
                            ...
                            word_checker_3(p, par_let,dim,par_vincoli,non_presente,presente_n,presente_pos_sbagliata,&ultimo_accesso_pos_sbgliata);
                            if(prima_attraversata == 0){
