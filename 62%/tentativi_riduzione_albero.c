            ...
            if(bt->left->vincolata == 1)
            {
                if(bt->left->right!=NULL)
                {
                    if(bt->left->right->vincolata==1)
                    {
                        ...
                        {
                            if(bt->left->right->right->vincolata==1)
                            {
                                if(bt->left->right->right->left!=NULL)
                                    applica_vincoli_vettori(bt->left->right->right->left,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                                if(bt->left->right->right->right!=NULL)
                                    applica_vincoli_vettori(bt->left->right->right->right,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                            }
                            else
                        ...
                        {
                            if(bt->left->right->left->vincolata==1)
                            {
                                if(bt->left->right->left->right!=NULL)
                                    applica_vincoli_vettori(bt->left->right->left->right,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                                if(bt->left->right->left->left!=NULL)
                                    applica_vincoli_vettori(bt->left->right->left->left,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                            }else
                ...
                if(bt->left->left!=NULL)
                {
                    if(bt->left->left->vincolata==1)
                    {
                        ...
                        {
                            if(bt->left->left->right->vincolata == 1)
                            {
                                if(bt->left->left->right->left!=NULL)
                                    applica_vincoli_vettori(bt->left->left->right->left,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                                if(bt->left->left->right->right!=NULL)
                                    applica_vincoli_vettori(bt->left->left->right->right,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                            }else
                        ...
                        {
                            if(bt->left->left->left->vincolata == 1)
                            {
                                if(bt->left->left->left->right!=NULL)
                                    applica_vincoli_vettori(bt->left->left->left->right,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                                if(bt->left->left->left->left!=NULL)
                                    applica_vincoli_vettori(bt->left->left->left->left,dim,vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
                            }
                            else
            ...
            if (bt->right->vincolata == 1) {
                if(bt->right->right!=NULL)
                {
                    if(bt->right->right->vincolata == 1)
                    {
                        ...
                        {
                            if(bt->right->right->right->vincolata == 1)
                            {
                                if(bt->right->right->right->right!=NULL)
                                    applica_vincoli_vettori(bt->right->right->right->right, dim, vincoli, non_presente, presente_n,
                                                            presente_pos_sbagliata, conta_filtrate);
                                if(bt->right->right->right->left!=NULL)
                                    applica_vincoli_vettori(bt->right->right->right->left, dim, vincoli, non_presente, presente_n,
                                                            presente_pos_sbagliata, conta_filtrate);
                            }else
                        ...
                        {
                            if(bt->right->right->left->vincolata == 1)
                            {
                                if(bt->right->right->left->left!=NULL)
                                    applica_vincoli_vettori(bt->right->right->left->left, dim, vincoli, non_presente, presente_n,
                                                            presente_pos_sbagliata, conta_filtrate);
                                if(bt->right->right->left->right!=NULL)
                                    applica_vincoli_vettori(bt->right->right->left->right, dim, vincoli, non_presente, presente_n,
                                                            presente_pos_sbagliata, conta_filtrate);
                            }else{
                ...
                if(bt->right->left!=NULL)
                {
                    if(bt->right->left->vincolata == 1)
                    {
                        ...
                        {
                            if(bt->right->left->right->vincolata==1)
                            {
                                if(bt->right->left->right->right!=NULL)
                                    applica_vincoli_vettori(bt->right->left->right->right, dim, vincoli, non_presente, presente_n, presente_pos_sbagliata,
                                                            conta_filtrate);
                                if(bt->right->left->right->left!=NULL)
                                    applica_vincoli_vettori(bt->right->left->right->left, dim, vincoli, non_presente, presente_n, presente_pos_sbagliata,
                                                            conta_filtrate);
                            }else
                        ...
                        {
                            if(bt->right->left->left->vincolata == 1)
                            {
                                if(bt->right->left->left->right!=NULL)
                                    applica_vincoli_vettori(bt->right->left->left->right, dim, vincoli, non_presente, presente_n, presente_pos_sbagliata,
                                                            conta_filtrate);
                                if(bt->right->left->left->left!=NULL)
                                    applica_vincoli_vettori(bt->right->left->left->left, dim, vincoli, non_presente, presente_n, presente_pos_sbagliata,
                                                            conta_filtrate);
                            }else
