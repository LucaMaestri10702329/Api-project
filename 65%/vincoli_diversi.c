    ...
    if(bt != NULL) {
        ...
        while(i<dim-1) {
            if (non_presente[ritorna_posizione(bt->key[i])] == 1 ||
                presente_pos_sbagliata[i][ritorna_posizione(bt->key[i])] == 1) {
                vincolata = 1;
                break;
            }
...
int main() {
    ...
    long start_search_t, end_search_t;
    long start_add_t, end_add_t;
    long start_add_par_t, end_add_par_t;
    long start_print_t, end_print_t;
    long start_wc3_t, end_wc3_t;
    ...
    while(1) {
        ...
        end_add_t = get_nanos();
        printf("Tempo di esecuzione aggiunta (ms): %.2lf\n", (end_add_t - start_add_t) / (float)1000000);
        ...
        if(c==EOF) {
            ...
            while (tent > 0) {
                ...
                if (par_let[0] == '+') {
                    if (par_let[1] == 's') {
                        start_print_t = get_nanos();
                        if(prima_attraversata==0)
                            MorrisTraversal(bt);
                        else
                            MorrisTraversal(filtrate);
                        end_print_t = get_nanos();
                        printf("Tempo di esecuzione stampa (ms): %.2lf\n", (end_print_t - start_print_t) / (float)1000000);
                    }
                    else {
                        start_add_par_t = get_nanos();
                        if (par_let[1] == 'i') {
                        ...
                        end_add_par_t = get_nanos();
                        printf("Tempo di esecuzione aggiunta in partita (ms): %.2lf\n", (end_add_par_t - start_add_par_t) / (float)1000000);
                    }

                } else {
                    start_search_t = get_nanos();
                    if (ricerca_parola_2(bt, par_let,dim) == NULL)
                    {
                        ...
                        if (strcmp_home_made(p, par_let) == 0) {
                            ...
                            start_wc3_t = get_nanos();
                            word_checker_3(p, par_let,dim,par_vincoli,non_presente,presente_n,presente_pos_sbagliata);
                            end_wc3_t = get_nanos();
                            printf("Tempo di esecuzione word checker (ms): %.2lf\n", (end_wc3_t - start_wc3_t) / (float)1000000);
                            if(prima_attraversata == 0){
                    ...
                    end_search_t = get_nanos();
                    printf("Tempo di esecuzione ricerca tent %d (ms): %.2lf\n", tent, (end_search_t - start_search_t) / (float)1000000);
                }
            }
            free(par_read);
