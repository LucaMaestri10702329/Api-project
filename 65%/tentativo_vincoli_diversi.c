...
node *create_node(char *input, int dim) {
    ...
    for(i = 0; i< dim-1; i++){
        ...
        for(j=0;j<dim-1;j++)
            if(input[i]==input[j])
                occorrenze ++;
        n->presente_n[i] = occorrenze;
    }
    return n;
}

node *create_node_noto(char *input, int dim, int occorrenze[]) {
    node *n = malloc(sizeof(node)+sizeof(char)*dim);
    n->left = NULL;
    n->right = NULL;
    memmove(n->key,input,dim);
    n->presente_n[0] = occorrenze[0];
    n->presente_n[1] = occorrenze[1];
    n->presente_n[2] = occorrenze[2];
    n->presente_n[3] = occorrenze[3];
    if(dim-2 >3)
        n->presente_n[4] = occorrenze[4];
...
node *BST_insert_iterative_noto(node *root, char *input, unsigned int dim, int occorrenze[]){
    node *temp = root;
    int condition = 1;

    if (root == NULL)
        return create_node_noto(input,dim,occorrenze);
    ...
    for(i=0;i<dim-1;i++) {
        if(output[i]=='+')
            ...
            if(output[i]=='|') {
                ...
                unsigned int m = 0;
                int pos_n = ritorna_posizione(word[i]);
                if(presente_n[1][pos_n]==-1 || presente_n[1][pos_n]<pres_word)
                    presente_n[1][pos_n] = pres_word;
                ...
                if(pres_rif == 0) {
                    ...
                    int pos_n = ritorna_posizione(word[i]);
                    if(presente_n[2][pos_n]==-1)
                        presente_n[2][pos_n] = pres_rif;
...
node * applica_vincoli_vettori(node * bt, node * temp, unsigned int dim, char *vincoli, int non_presente[],int presente_n[][64],int presente_pos_sbagliata[][64], int  conta_filtrate[]) {
    if(bt != NULL) {
        ...
        for (i = 0; i < dim - 1; i++) {
            ...
            unsigned int j=0;
                int pos = ritorna_posizione(bt->key[i]);
                if(presente_n[2][pos]!=-1 && bt->presente_n[i]!=presente_n[2][pos])
                {
                    vincolata = 1;
                    break;
                }
                if(presente_n[1][pos]!=-1 && bt->presente_n[i]!=presente_n[1][pos])
                {
                    vincolata = 1;
                    break;
                }
        ...
        if(vincolata == 1) {
            ...
            temp = BST_insert_iterative_noto(temp,bt->key,dim,bt->presente_n);
        if(bt->right!=NULL)
