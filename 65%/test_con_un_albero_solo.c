...
//calcola la posizione in un vettore di 64  in base al carattere in input
...
//una normale strcmp ma creata a mano
...
//visita in ordine dell'albero iterativa
...
//inserimento nell'albero iterativo
...
//inserimento dei puntatori in un secondo albero
node *BST_insert_iterative_secondo_albero(node *root, node *nodo){
    node *temp = root;
    int condition = 1;
    if (root == NULL)
        return nodo;
    ...
    while (condition) {
        if (strcmp_home_made(nodo->key,temp->key)>0) {
            if (temp->right == NULL) {
                temp->right = nodo;
                condition = 0;
            }
...
//versione migliorata della ricerca di una determinata parola in un albero
...
//elimina tutti gli elementi dell'albero
...
//filtra l'albero creandone uno nuovo
node * applica_vincoli_vettori(node * bt, node * temp, unsigned int dim, char *vincoli, int non_presente[],int presente_n[][64],int presente_pos_sbagliata[][64], int  conta_filtrate[]) {
    if(bt != NULL) {
        ...
        {
            temp = BST_insert_iterative_secondo_albero(temp,bt);
        }
...
//riprisitna i vincoli calcolati fin'ora
                    ...
                    if (ricerca_parola_2(bt, par_let,dim) == NULL)
                    {
                        ...
                        if (strcmp_home_made(p, par_let) == 0) {
                            ...
                            if(prima_attraversata == 0){
                                ...
                                filtrate = applica_vincoli_vettori(filtrate,NULL,dim,par_vincoli,non_presente,presente_n,presente_pos_sbagliata,conta_filtrate);
